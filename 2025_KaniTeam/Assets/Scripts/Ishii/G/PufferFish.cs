...
public class PufferFish : FishBase
{
    ...
    [SerializeField, Tooltip("膨らむ速度の調整用変数")]        float inflateRate = 0.1f;  // 膨らむ速度の調整用変数
    [SerializeField, Tooltip("最大膨張率")]                   float maxScale = 3.0f;     // 最大膨張率
    [SerializeField, Tooltip("大きいサイズの基準値")]          float largeSize = 2.0f;     // 大きいサイズの基準値
    ...
    protected override void Move()
    {
        if (isContact) return;
        // 落下時間に応じて膨らむ
        float scale = 1 + rb.linearVelocity.y * -inflateRate;
